...
module diagnosis.graph.render {
    ...
    const PARAMS = {
         ...
         */
        enableExtraction: false, // [@TS] No, do not do this!
        /**
    ...
    const nodeDisplayNameRegex = new RegExp(
        '^(?:' + diagnosis.graph.FUNCTION_LIBRARY_NODE_PREFIX +
        ')?(\\w+)_[a-z0-9]{8}(?:_\\d+)?$');
    ...
    export class RenderGraphInfo {
        ...
        getOrCreateRenderNodeByName(nodeName: string): RenderNodeInfo {
            ...
            renderInfo.isFadedOut = this.displayingStats &&
                !diagnosis.graph.util.hasDisplayableNodeStats(node.stats);

            var deviceHistogram = null;
            var xlaClusterHistogram = null;
            var opCompatibility = null;
            let diagnosisToolCompatibility: diagnosis.tools.Tool[] = null;

            /* Diagnosis works on group nodes exactly as on opNodes. Therefore, keep this out of if-statement. */
            if (renderInfo.node.type === NodeType.OP) {
                diagnosisToolCompatibility = diagnosis.tools.getApplicableTools((<OpNode>renderInfo.node).op);
            }
            ...
            if (opCompatibility != null) {
                renderInfo.compatibilityColors = [
                    {
                        color: diagnosis.graph.render.OpNodeColors.COMPATIBLE,
                        proportion: opCompatibility
                    },
                    {
                        color: diagnosis.graph.render.OpNodeColors.INCOMPATIBLE,
                        proportion: 1 - opCompatibility
                    }
                ];
            }
            if (diagnosisToolCompatibility != null) {
                renderInfo.diagnosisToolCompatibilityColors = [];
                diagnosisToolCompatibility.forEach((tool) => {
                    renderInfo.diagnosisToolCompatibilityColors.push({
                        color: diagnosis.tools.getToolSpec(tool).color,
                        proportion: 1 / diagnosisToolCompatibility.length
                    });
                });
            }
        ...
        private cloneFunctionLibraryMetanodeHelper(
            ...
            const newMetanode = diagnosis.graph.createMetanode(
        ...
        buildSubhierarchy(nodeName: string): void {
            ...
            if (!_.isEmpty(this.hierarchy.libraryFunctions)) {
                ...
                _.each(metagraph.nodes(), childName => {
                    ...
                    if (childName.indexOf(diagnosis.graph.FUNCTION_LIBRARY_NODE_PREFIX) === 0) {
            ...
            if (nodeName === diagnosis.graph.ROOT_NAME) {
        ...
        private buildSubhierarchiesForNeededFunctions(
            ...
            _.each(metagraph.edges(), edgeObj => {
                ...
                _.forEach(renderMetaedgeInfo.metaedge.baseEdgeList,
                    baseEdge => {
                        const sourcePathList = baseEdge.v.split(diagnosis.graph.NAMESPACE_DELIM);

                        for (let i = sourcePathList.length; i >= 0; i--) {
                            const fromBeginningPathList = sourcePathList.slice(0, i);
                            const node = this.hierarchy.node(
                                fromBeginningPathList.join(diagnosis.graph.NAMESPACE_DELIM));
                            if (node) {
                                if (node.type === NodeType.OP &&
                                    ...
                                    for (let j = 1; j < fromBeginningPathList.length; j++) {
                                        ...
                                        const currentNodeName = fromBeginningPathList
                                            .slice(0, j).join(diagnosis.graph.NAMESPACE_DELIM);
                                        if (!currentNodeName) {
                                            continue;
                                        }
    ...
    export class AnnotationList {
        ...
        push(annotation: Annotation): void {
            ...
            let ellipsisNode = new diagnosis.graph.EllipsisNodeImpl(1);
    ...
    export class RenderNodeInfo {
        ...
        /**
         * [@TS] List of (color, proportion) tuples based an the type of diagnosis tool
         * that can be applied on this node.
         */
        diagnosisToolCompatibilityColors: Array<{ color: string, proportion: number }>;
        ...
        constructor(node: Node) {
            ...
            this.displayName = node.name.substring(
                node.name.lastIndexOf(diagnosis.graph.NAMESPACE_DELIM) + 1);
            ...
            if (node.type === NodeType.META &&
                ...
                if (match) {
                    ...
                    this.displayName, diagnosis.graph.FUNCTION_LIBRARY_NODE_PREFIX)) {
                    ...
                    this.displayName = this.displayName.substring(
                        diagnosis.graph.FUNCTION_LIBRARY_NODE_PREFIX.length);
                }
            }
        }
    ...
    export class RenderMetaedgeInfo {
         ...
         * @see diagnosis.graph.node.subsceneAdjustPaths
         */
        points: Point[];
    ...
    function extractSpecifiedNodes(renderNode: RenderGroupNodeInfo) {
        ...
        _.each(graph.nodes(), n => {
            ...
            if (renderInfo.node.include === InclusionType.EXCLUDE &&
                !n.startsWith(diagnosis.graph.FUNCTION_LIBRARY_NODE_PREFIX)) {
...
} // close module diagnosis.graph.render
