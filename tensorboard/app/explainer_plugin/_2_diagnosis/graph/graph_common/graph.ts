...
export class OpNodeImpl implements OpNode {
   ...
   */
  constructor(rawNode: diagnosis.graph.proto.NodeDef) {
    this.op = rawNode.op;
...
export function joinStatsInfoWithGraph(
    graph: SlimGraph, stats: diagnosis.graph.proto.StepStats,
...
export function build(
    graphDef: diagnosis.graph.proto.GraphDef, params: BuildParams,
  ...
  return diagnosis.graph.util
      .runAsyncTask(
          ...
          () => {
            ...
            const processFunction = (func: diagnosis.graph.proto.FunctionDef) => {
...
export function getIncludeNodeButtonString(include: InclusionType) {
  if (include === diagnosis.graph.InclusionType.EXCLUDE) {
...
export function getGroupSeriesNodeButtonString(group: SeriesGroupingType) {
  if (group === diagnosis.graph.SeriesGroupingType.GROUP) {
...
export function toggleNodeSeriesGroup(
  map: { [name: string]: diagnosis.graph.SeriesGroupingType }, name: string) {
  if (!(name in map) || map[name] === diagnosis.graph.SeriesGroupingType.GROUP) {
    map[name] = diagnosis.graph.SeriesGroupingType.UNGROUP;
  } else {
    map[name] = diagnosis.graph.SeriesGroupingType.GROUP;
  }
};

} // close module diagnosis.graph
