 ...
 */
module diagnosis.graph.hierarchy {
...
export interface HierarchyParams {
  ...
  seriesMap: { [name: string]: diagnosis.graph.SeriesGroupingType };
 ...
 */
export function build(graph: diagnosis.graph.SlimGraph, params: HierarchyParams,
  ...
  return diagnosis.graph.util
      .runAsyncTask(
          ...
          tracker)
      .then(() => {
        return diagnosis.graph.util.runAsyncTask('Detect series', 20, () => {
      ...
      .then(() => {
        return diagnosis.graph.util.runAsyncTask('Adding edges', 30, () => {
          addEdges(h, graph, seriesNames);
        }, tracker);
      })
      .then(() => {
        return diagnosis.graph.util.runAsyncTask(
...
export function joinAndAggregateStats(
    h: Hierarchy, stats: diagnosis.graph.proto.StepStats) {
  // Get all the possible device and XLA cluster names.
  let deviceNames = {};
  let xlaClusterNames = {};
      ...
      if (!opNode.compatible) {
        if (opNode.owningSeries) {
          if (hierarchyParams &&
              ...
              === diagnosis.graph.SeriesGroupingType.UNGROUP) {
  ...
  _.each(graph.nodes, (node, nodeName) => {
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      if (!child) {
        ...
        if (name.indexOf(diagnosis.graph.FUNCTION_LIBRARY_NODE_PREFIX) === 0 &&
            parent.name === diagnosis.graph.ROOT_NAME) {
...
function groupSeries(metanode: Metanode, hierarchy: Hierarchy,
    ...
    map: { [name: string]: diagnosis.graph.SeriesGroupingType },
    useGeneralizedSeriesPatterns: boolean) {
  let metagraph = metanode.metagraph;
  _.each(metagraph.nodes(), n => {
    let child = metagraph.node(n);
    if (child.type === diagnosis.graph.NodeType.META) {
      groupSeries(
          <Metanode>child,
          hierarchy,
          seriesNames,
  ...
  _.each(seriesDict, function(seriesNode: SeriesNode, seriesName: string) {
    ...
    if (nodeMemberNames.length < threshold && !(seriesNode.name in map)) {
      map[seriesNode.name] = diagnosis.graph.SeriesGroupingType.UNGROUP;
    }
...
} // close module diagnosis.graph.hierarchy
