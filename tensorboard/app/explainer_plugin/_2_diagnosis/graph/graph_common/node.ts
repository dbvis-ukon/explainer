...
==============================================================================*/
module diagnosis.graph.scene.node {
    import RenderNodeInfo = diagnosis.graph.render.RenderNodeInfo;
    ...
    export function getGroupSettingLabel(node: Node) {
        return diagnosis.graph.getGroupSeriesNodeButtonString(
            getContainingSeries(node) !== null ? diagnosis.graph.SeriesGroupingType.GROUP :
    ...
    function getGradient(
        ...
        let escapedId = diagnosis.graph.util.escapeQuerySelector(id);
     ...
     * option.
     */
    export function getFillForNode(templateIndex, colorBy, selectedTool : diagnosis.tools.Tool,
        ...
        switch (colorBy) {
            ...
            case ColorBy.INNSPECTOR_TOOL_COMPATIBILITY:
                if (selectedTool !== diagnosis.tools.Tool.NONE) {
                    /* Tool is selected. Only show, if it can be applied to node or not. */

                    if (renderInfo.node.type === NodeType.OP) {
                        const applicableTools = diagnosis.tools.getApplicableTools((<OpNode>renderInfo.node).op);

                        if (applicableTools.indexOf(selectedTool) > -1) {
                            return diagnosis.tools.getToolSpec(selectedTool).color;
                        }
                        else {
                            return "#FFFFFF";
                        }
                    ...
                    if (renderInfo.diagnosisToolCompatibilityColors == null) {
                        // Return the hue for unknown compatibility info.
                        return colorParams.UNKNOWN;
                    }
                    return isExpanded ? colorParams.EXPANDED_COLOR :
                        getGradient(
                            'diagnosis-tool-compat-' + renderInfo.node.name,
     ...
     */
    export function traceInputs(renderGraphInfo: diagnosis.graph.render.RenderGraphInfo) {
        // Reset all styling.
    ...
    export function _getAllContainedOpNodes(
        nodeName: string, renderGraphInfo: diagnosis.graph.render.RenderGraphInfo) {
        let opNodes = [];

        // Get current node.
        let node = renderGraphInfo.getNodeByName(nodeName) as diagnosis.graph.GroupNode |
            diagnosis.graph.OpNode;

        // If node is already OpNode then return the node plus its input embeddings.
        if (node instanceof diagnosis.graph.OpNodeImpl) {
            return [node].concat(node.inEmbeddings);
        }

        // Otherwise, make recursive call for each node contained by the GroupNode.
        let childNodeNames = (node as diagnosis.graph.GroupNode).metagraph.nodes();
    ...
    export function traceAllInputsOfOpNode(
        renderGraphInfo: diagnosis.graph.render.RenderGraphInfo, startNode: OpNode,
        ...
        _.each(inputs, function (nodeInstance) {
            ...
            if (resolvedNode instanceof MetanodeImpl) {
                let resolvedNodeName = diagnosis.graph.getStrictName(resolvedNode.name);
        ...
        for (let index = 1; currentNode.name !== diagnosis.graph.ROOT_NAME; index++) {
    ...
    function _markParentsOfNodes(visibleNodes: { [nodeName: string]: Node }) {
        _.forOwn(visibleNodes, function (nodeInstance: Node) {
            ...
            while (currentNode.name !== diagnosis.graph.ROOT_NAME) {
    ...
    export function getVisibleParent(
        renderGraphInfo: diagnosis.graph.render.RenderGraphInfo,
        currentNode: diagnosis.graph.Node) {
        let found = false;
