...
export class OpNodeImpl implements OpNode {
   ...
   */
  constructor(rawNode: understanding.graph.proto.NodeDef) {
    this.op = rawNode.op;
...
export function joinStatsInfoWithGraph(
    graph: SlimGraph, stats: understanding.graph.proto.StepStats,
...
export function build(
    graphDef: understanding.graph.proto.GraphDef, params: BuildParams,
  ...
  return understanding.graph.util
      .runAsyncTask(
          ...
          () => {
            ...
            const processFunction = (func: understanding.graph.proto.FunctionDef) => {
...
export function getIncludeNodeButtonString(include: InclusionType) {
  if (include === understanding.graph.InclusionType.EXCLUDE) {
...
export function getGroupSeriesNodeButtonString(group: SeriesGroupingType) {
  if (group === understanding.graph.SeriesGroupingType.GROUP) {
...
export function toggleNodeSeriesGroup(
  map: { [name: string]: understanding.graph.SeriesGroupingType }, name: string) {
  if (!(name in map) || map[name] === understanding.graph.SeriesGroupingType.GROUP) {
    map[name] = understanding.graph.SeriesGroupingType.UNGROUP;
  } else {
    map[name] = understanding.graph.SeriesGroupingType.GROUP;
  }
};

} // close module understanding.graph
