...
==============================================================================*/
module understanding.graph.scene.node {
    import RenderNodeInfo = understanding.graph.render.RenderNodeInfo;
    ...
    export function buildGroup(
        ...
        let nodeGroups =
            (container as any).selectAll(function () {
        ...
        selection
            ...
            .on('click',
                d => {
                    ...
                    // a graph-select.
                    const event = <KeyboardEvent>d3.event;

                    event.stopPropagation();

                    if(event.ctrlKey) {
                        sceneElement.fire('node-toggle-extract', {name: d.node.name});
                    } else {
    ...
    export function getGroupSettingLabel(node: Node) {
        return understanding.graph.getGroupSeriesNodeButtonString(
            getContainingSeries(node) !== null ? understanding.graph.SeriesGroupingType.GROUP :
    ...
    export enum ColorBy {
        STRUCTURE, DEVICE, XLA_CLUSTER, COMPUTE_TIME, MEMORY,
        OP_COMPATIBILITY, INNSPECTOR_TOOL_COMPATIBILITY
    };

    function getGradient(
        id: string, colors: Array<{ color: string, proportion: number }>) {
        let escapedId = understanding.graph.util.escapeQuerySelector(id);
     ...
     * option.
     */
    export function getFillForNode(templateIndex, colorBy, selectedTool : understanding.tools.Tool,
        ...
        switch (colorBy) {
            ...
            case ColorBy.INNSPECTOR_TOOL_COMPATIBILITY:
                if (selectedTool !== understanding.tools.Tool.NONE) {
                    /* Tool is selected. Only show, if it can be applied to node or not. */

                    if (renderInfo.node.type === NodeType.OP) {
                        const applicableTools = understanding.tools.getApplicableTools((<OpNode>renderInfo.node).op);

                        if (applicableTools.indexOf(selectedTool) > -1) {
                            return understanding.tools.getToolSpec(selectedTool).color;
                        }
                        else {
                            return "#FFFFFF";
                        }
                    ...
                    /* No Tool is selected. For each node show, which tools could be applied. */

                    if (renderInfo.understandingToolCompatibilityColors == null) {
                        // Return the hue for unknown compatibility info.
                        return colorParams.UNKNOWN;
                    }
                    return isExpanded ? colorParams.EXPANDED_COLOR :
                        getGradient(
                            'understanding-tool-compat-' + renderInfo.node.name,
    ...
    export function stylize(nodeGroup, renderInfo: render.RenderNodeInfo,
        ...
        let fillColor = getFillForNode(sceneElement.templateIndex,
            ColorBy[sceneElement.colorBy.toUpperCase()], sceneElement.selectedTool,
     ...
     */
    export function traceInputs(renderGraphInfo: understanding.graph.render.RenderGraphInfo) {
        // Reset all styling.
    ...
    export function _getAllContainedOpNodes(
        nodeName: string, renderGraphInfo: understanding.graph.render.RenderGraphInfo) {
        let opNodes = [];

        // Get current node.
        let node = renderGraphInfo.getNodeByName(nodeName) as understanding.graph.GroupNode |
            understanding.graph.OpNode;

        // If node is already OpNode then return the node plus its input embeddings.
        if (node instanceof understanding.graph.OpNodeImpl) {
            return [node].concat(node.inEmbeddings);
        }

        // Otherwise, make recursive call for each node contained by the GroupNode.
        let childNodeNames = (node as understanding.graph.GroupNode).metagraph.nodes();
    ...
    export function traceAllInputsOfOpNode(
        renderGraphInfo: understanding.graph.render.RenderGraphInfo, startNode: OpNode,
        ...
        _.each(inputs, function (nodeInstance) {
            ...
            if (resolvedNode instanceof MetanodeImpl) {
                let resolvedNodeName = understanding.graph.getStrictName(resolvedNode.name);
        ...
        for (let index = 1; currentNode.name !== understanding.graph.ROOT_NAME; index++) {
    ...
    function _markParentsOfNodes(visibleNodes: { [nodeName: string]: Node }) {
        _.forOwn(visibleNodes, function (nodeInstance: Node) {
            ...
            while (currentNode.name !== understanding.graph.ROOT_NAME) {
    ...
    export function getVisibleParent(
        renderGraphInfo: understanding.graph.render.RenderGraphInfo,
        currentNode: understanding.graph.Node) {
        let found = false;
