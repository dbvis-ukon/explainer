 ...
 */
module understanding.graph.render {
    ...
    export let OpNodeColors = {
        DEFAULT_FILL: '#ffffff', DEFAULT_STROKE: '#b2b2b2',
    ...
    const nodeDisplayNameRegex = new RegExp(
        '^(?:' + understanding.graph.FUNCTION_LIBRARY_NODE_PREFIX +
        ')?(\\w+)_[a-z0-9]{8}(?:_\\d+)?$');
    ...
    export class RenderGraphInfo {
        ...
        getOrCreateRenderNodeByName(nodeName: string): RenderNodeInfo {
            ...
            renderInfo.isFadedOut = this.displayingStats &&
                !understanding.graph.util.hasDisplayableNodeStats(node.stats);

            var deviceHistogram = null;
            var xlaClusterHistogram = null;
            var opCompatibility = null;
            let understandingToolCompatibility: understanding.tools.Tool[] = null;

            /* Understanding works on group nodes exactly as on opNodes. Therefore, keep this out of if-statement. */
            if (renderInfo.node.type === NodeType.OP) {
                understandingToolCompatibility = understanding.tools.getApplicableTools((<OpNode>renderInfo.node).op);
            }
            ...
            if (opCompatibility != null) {
                renderInfo.compatibilityColors = [
                    {
                        color: understanding.graph.render.OpNodeColors.COMPATIBLE,
                        proportion: opCompatibility
                    },
                    {
                        color: understanding.graph.render.OpNodeColors.INCOMPATIBLE,
                        proportion: 1 - opCompatibility
                    }
                ];
            }
            if (understandingToolCompatibility != null) {
                renderInfo.understandingToolCompatibilityColors = [];
                understandingToolCompatibility.forEach((tool) => {
                    renderInfo.understandingToolCompatibilityColors.push({
                        color: understanding.tools.getToolSpec(tool).color,
                        proportion: 1 / understandingToolCompatibility.length
                    });
                });
            }
        ...
        private cloneFunctionLibraryMetanodeHelper(
            ...
            const newMetanode = understanding.graph.createMetanode(
        ...
        buildSubhierarchy(nodeName: string): void {
            ...
            if (!_.isEmpty(this.hierarchy.libraryFunctions)) {
                ...
                _.each(metagraph.nodes(), childName => {
                    ...
                    if (childName.indexOf(understanding.graph.FUNCTION_LIBRARY_NODE_PREFIX) === 0) {
            ...
            if (nodeName === understanding.graph.ROOT_NAME) {
        ...
        private buildSubhierarchiesForNeededFunctions(
            ...
            _.each(metagraph.edges(), edgeObj => {
                ...
                _.forEach(renderMetaedgeInfo.metaedge.baseEdgeList,
                    baseEdge => {
                        const sourcePathList = baseEdge.v.split(understanding.graph.NAMESPACE_DELIM);

                        for (let i = sourcePathList.length; i >= 0; i--) {
                            const fromBeginningPathList = sourcePathList.slice(0, i);
                            const node = this.hierarchy.node(
                                fromBeginningPathList.join(understanding.graph.NAMESPACE_DELIM));
                            if (node) {
                                if (node.type === NodeType.OP &&
                                    ...
                                    for (let j = 1; j < fromBeginningPathList.length; j++) {
                                        ...
                                        const currentNodeName = fromBeginningPathList
                                            .slice(0, j).join(understanding.graph.NAMESPACE_DELIM);
                                        if (!currentNodeName) {
                                            continue;
                                        }
    ...
    export class AnnotationList {
        ...
        push(annotation: Annotation): void {
            ...
            let ellipsisNode = new understanding.graph.EllipsisNodeImpl(1);
    ...
    export class RenderNodeInfo {
        ...
        /**
         * [@TS] List of (color, proportion) tuples based an the type of understanding tool
         * that can be applied on this node.
         */
        understandingToolCompatibilityColors: Array<{ color: string, proportion: number }>;
        ...
        constructor(node: Node) {
            ...
            this.displayName = node.name.substring(
                node.name.lastIndexOf(understanding.graph.NAMESPACE_DELIM) + 1);
            ...
            if (node.type === NodeType.META &&
                ...
                if (match) {
                    ...
                    this.displayName, understanding.graph.FUNCTION_LIBRARY_NODE_PREFIX)) {
                    ...
                    this.displayName = this.displayName.substring(
                        understanding.graph.FUNCTION_LIBRARY_NODE_PREFIX.length);
                }
            }
        }
    ...
    export class RenderMetaedgeInfo {
         ...
         * @see understanding.graph.node.subsceneAdjustPaths
         */
        points: Point[];
    ...
    function extractSpecifiedNodes(renderNode: RenderGroupNodeInfo) {
        ...
        _.each(graph.nodes(), n => {
            ...
            if (renderInfo.node.include === InclusionType.EXCLUDE &&
                !n.startsWith(understanding.graph.FUNCTION_LIBRARY_NODE_PREFIX)) {
...
} // close module understanding.graph.render
