 ...
 */
module understanding.graph.hierarchy {
...
export interface HierarchyParams {
  ...
  seriesMap: { [name: string]: understanding.graph.SeriesGroupingType };
 ...
 */
export function build(graph: understanding.graph.SlimGraph, params: HierarchyParams,
  ...
  return understanding.graph.util
      .runAsyncTask(
          'Adding nodes', 20,
          () => {
          ...
          tracker)
      .then(() => {
        return understanding.graph.util.runAsyncTask('Detect series', 20, () => {
      ...
      .then(() => {
        return understanding.graph.util.runAsyncTask('Adding edges', 30, () => {
          addEdges(h, graph, seriesNames);
        }, tracker);
      })
      .then(() => {
        return understanding.graph.util.runAsyncTask(
...
export function joinAndAggregateStats(
    h: Hierarchy, stats: understanding.graph.proto.StepStats) {
  // Get all the possible device and XLA cluster names.
  let deviceNames = {};
  let xlaClusterNames = {};
      ...
      if (!opNode.compatible) {
        if (opNode.owningSeries) {
          if (hierarchyParams &&
              ...
              === understanding.graph.SeriesGroupingType.UNGROUP) {
  ...
  _.each(graph.nodes, (node, nodeName) => {
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      if (!child) {
        ...
        if (name.indexOf(understanding.graph.FUNCTION_LIBRARY_NODE_PREFIX) === 0 &&
            parent.name === understanding.graph.ROOT_NAME) {
...
function groupSeries(metanode: Metanode, hierarchy: Hierarchy,
    ...
    map: { [name: string]: understanding.graph.SeriesGroupingType },
    useGeneralizedSeriesPatterns: boolean) {
  let metagraph = metanode.metagraph;
  _.each(metagraph.nodes(), n => {
    let child = metagraph.node(n);
    if (child.type === understanding.graph.NodeType.META) {
      groupSeries(
          <Metanode>child,
          hierarchy,
          seriesNames,
  ...
  _.each(seriesDict, function(seriesNode: SeriesNode, seriesName: string) {
    ...
    if (nodeMemberNames.length < threshold && !(seriesNode.name in map)) {
      map[seriesNode.name] = understanding.graph.SeriesGroupingType.UNGROUP;
    }
...
} // close module understanding.graph.hierarchy
